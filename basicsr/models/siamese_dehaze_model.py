...
class SiameseDehazeModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.LQ_stage:
            with torch.no_grad():
                self.gt_rec, _, _, _, gt_indices = self.net_hq(self.gt)

            self.output_1, self.feature_1, l_codebook_1, l_semantic_1, _ = self.net_g(self.lq1, gt_indices)
            self.output_2, self.feature_2, l_codebook_2, l_semantic_2, _ = self.net_g(self.lq2, gt_indices) 
        else:
        ...
        if train_opt.get('codebook_opt', None):
            l_codebook_1 *= train_opt['codebook_opt']['loss_weight'] 
            l_codebook_2 *= train_opt['codebook_opt']['loss_weight']
            l_g_total += l_codebook_1.mean()
            l_g_total += l_codebook_2.mean()
            loss_dict['l_codebook'] = (l_codebook_1 + l_codebook_2).mean()

        # semantic cluster loss, only for LQ stage!
        if train_opt.get('semantic_opt', None) and isinstance(l_semantic_1, torch.Tensor):
            l_semantic_1 *= train_opt['semantic_opt']['loss_weight']
            l_semantic_2 *= train_opt['semantic_opt']['loss_weight']  
            l_g_total += l_semantic_1.mean()
            l_g_total += l_semantic_2.mean()
            loss_dict['l_semantic'] = (l_semantic_1 + l_semantic_2).mean()

        # pixel loss
        if self.cri_pix:
            l_pix = self.cri_pix(self.output_1, self.gt) + self.cri_pix(self.output_2, self.gt)
            l_g_total += l_pix
            loss_dict['l_pix'] = l_pix

        # perceptual loss
        if self.cri_perceptual:
            l_percep_1, l_style_1 = self.cri_perceptual(self.output_1, self.gt)
            l_percep_2, l_style_2 = self.cri_perceptual(self.output_2, self.gt)
            
            if l_percep_1 is not None:
                l_percep = l_percep_1 + l_percep_2
                l_g_total += l_percep.mean()
                loss_dict['l_percep'] = l_percep.mean()
            if l_style_1 is not None:
                l_style = l_style_1 + l_style_2
                l_g_total += l_style
                loss_dict['l_style'] = l_style
        
        # KD loss
        if self.cri_kd:
            l_kd = self.cri_kd(self.feature_1, self.feature_2)
            l_kd *= train_opt['kd_opt']['loss_weight']
            l_g_total += l_kd
            loss_dict['l_kd'] = l_kd

        # gan loss
        if self.use_dis and current_iter > train_opt['net_d_init_iters']:
            fake_g_pred = (self.net_d(self.output_1) + self.net_d(self.output_2)) / 2
        ...
        if not self.fixed_disc and self.use_dis and current_iter > train_opt['net_d_init_iters']:
            ...
            l_d_real.backward()
            # fake
            fake_d_pred = (self.net_d(self.output_1.detach()) + self.net_d(self.output_2.detach())) / 2
    ...
    def get_current_visuals(self):
        ...
        out_dict['lq_1'] = self.lq1.detach().cpu()[:vis_samples]
        out_dict['lq_2'] = self.lq2.detach().cpu()[:vis_samples]
        out_dict['result_1'] = self.output_1.detach().cpu()[:vis_samples]
        out_dict['result_2'] = self.output_2.detach().cpu()[:vis_samples]
