...
class MultiScaleDecoder(nn.Module):
    ...
    def forward(self, input, code_decoder_output, encoder_output):
        x = input
        for idx, m in enumerate(self.upsampler):
            with torch.backends.cudnn.flags(enabled=False):
                if not self.only_residual:
                    x = m(x + encoder_output[idx]) + code_decoder_output[idx]
                else:
                    x = m(x)
...
class VQUNetDehazeNet(nn.Module):
    def __init__(self,
                 *,
                 in_channel=3,
    ...
    def encode_and_decode(self, input, gt_indices=None, current_iter=None):
        ...
        if self.LQ_stage:
            residual_feature = self.multiscale_decoder(enc_feats[0], code_decoder_output, enc_feats[1:])
