...
class DehazeNet(nn.Module):
    def __init__(self,
        ...
        self.multiscale_encoder = MultiScaleEncoder(
                                ...
                                channel_query_dict,
                                norm_type, act_type, LQ_stage, only_residual
                            )
    ...
    def encode_and_decode(self, input, gt_indices=None, current_iter=None):
        ...
        for i in range(self.max_depth):
            ...
            if cur_res in self.codebook_scale:  # needs to perform quantize
                ...
                # print(before_quant_feat.shape)
                feat_to_quant = self.before_quant_group[quant_idx](before_quant_feat)

                if gt_indices is not None:
                    # print(feat_to_quant.shape, gt_indices[quant_idx].shape)
        ...
        if self.LQ_stage and self.only_residual:
            out_img = self.out_conv(enc_feats[-1])
        else:
        ...
        return out_img, feat_to_quant, codebook_loss, semantic_loss, indices_list
    ...
    def forward(self, input, gt_indices=None):
        if gt_indices is not None:
            ...
            dec, feat_to_quant, codebook_loss, semantic_loss, indices = self.encode_and_decode(input, gt_indices)
        else:
            # in HQ stage, or LQ test stage, no GT indices needed.
            dec, feat_to_quant, codebook_loss, semantic_loss, indices = self.encode_and_decode(input)

        return dec, feat_to_quant, codebook_loss, semantic_loss, indices
