...
class WarpBlock(nn.Module):
    ...
    def forward(self, x_vq, x_residual):
        offset = self.offset(torch.cat([x_vq, x_residual], dim=1))
        feat_after_warp = self.dcn(x_vq, offset)
...
class VQWarpDehazeNet(nn.Module):
    def __init__(self,
                 *,
                 in_channel=3,
    ...
    def test(self, input):
        ...
        dec, _, _, _, index = self.encode_and_decode(input)
