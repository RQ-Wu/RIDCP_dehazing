        ...
        for idx, m in enumerate(self.upsampler):
            with torch.backends.cudnn.flags(enabled=False):
                if self.only_residual:
                    code_input_downsample = nn.functional.interpolate(code_decoder_output[idx], scale_factor=0.125)
                    code_input_upsample = nn.functional.interpolate(code_input_downsample, scale_factor=8)
                    x = m(x) + code_input_upsample
                else:
                    x = m(x)
...
class VQSampleDehazeNet(nn.Module):
    def __init__(self,
                 *,
                 in_channel=3,
        ...
        if self.LQ_stage:
            self.multiscale_decoder = MultiScaleDecoder(
                                ...
                                self.gt_res // self.scale_factor, 
                                channel_query_dict,
                                norm_type, act_type, only_residual
            )
        ...
        self.out_conv_residual = nn.Conv2d(out_ch, 3, 3, 1, 1)
    ...
    def encode_and_decode(self, input, gt_indices=None, current_iter=None):
        enc_feats = self.multiscale_encoder(input.detach())[::-1]
        ...
        if self.LQ_stage:
            residual_feature = self.multiscale_decoder(enc_feats[0], code_decoder_output)
            out_img_residual = self.out_conv_residual(residual_feature)

        codebook_loss = sum(codebook_loss_list)
        semantic_loss = sum(semantic_loss_list) if len(semantic_loss_list) else codebook_loss * 0

        return out_img, out_img_residual, codebook_loss, semantic_loss, indices_list
    ...
    def test(self, input):
        ...
        _, dec, _, _, _ = self.encode_and_decode(input)
    ...
    def forward(self, input, gt_indices=None):
        ...
        if gt_indices is not None:
            ...
            dec, dec_residual, codebook_loss, semantic_loss, indices = self.encode_and_decode(input, gt_indices)
        else:
            # in HQ stage, or LQ test stage, no GT indices needed.
            dec, dec_residual, codebook_loss, semantic_loss, indices = self.encode_and_decode(input)

        return dec, dec_residual, codebook_loss, semantic_loss, indices
