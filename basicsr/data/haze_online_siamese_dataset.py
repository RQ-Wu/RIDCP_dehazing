...
def add_haze(img, img_depth, beta_range, A_range, color_p, color_range):
    img_lq = img.copy()
    # adjust luminance
    ...
    # add haze
    beta = np.random.rand(1) * (beta_range[1] - beta_range[0]) + beta_range[0]
    t = np.exp(-(1- img_depth) * 2.0 * beta)
    t = t[:, :, np.newaxis]
    
    A = np.random.rand(1) * (A_range[1] - A_range[0]) + A_range[0]
    if np.random.rand(1) < color_p:
...
class HazeSiameseDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(HazeSiameseDataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        img_lq1 = add_haze(img_gt, img_depth, self.beta_range, self.A_range, self.color_p, self.color_range)
        img_lq2 = add_haze(img_gt, img_depth, self.beta_range, self.A_range, self.color_p, self.color_range)

        # handle images with 4 channels 
        if img_gt.shape[-1] > 3:
            img_gt = img_gt[:, :, :3]
            img_lq1 = img_lq1[:, :, :3]
            img_lq2 = img_lq2[:, :, :3]
        ...
        if self.opt['phase'] == 'train':
            ...
            input_lq_size = np.min(img_lq1.shape[:2])
            ...
            if self.opt['use_resize_crop']:
                ...
                img_lq1 = random_resize(img_lq1, resize_factor)
                img_lq2 = random_resize(img_lq2, resize_factor)

                # random crop
                img_gt, img_lq = paired_random_crop(img_gt, [img_lq1, img_lq2], gt_size, input_gt_size // input_lq_size,
                                               gt_path)
                
                img_lq1, img_lq2 = img_lq
            # flip, rotation
            img_gt, img_lq1, img_lq2 = augment([img_gt, img_lq1, img_lq2], self.opt['use_flip'],
                                     self.opt['use_rot'])
